...
In **PDF Parser**, opening a library file (Browse Files or a Discover link) fills the controls from its `scan_config` (crop, language, engine, model, multi-page). If the config has `sub_sections`, a **Verify sub-sections** button opens a viewer with each sub-section's start and end page side by side; step with the arrow keys, tick a few, or take a random 10. Each pane also has `◀ page` / `page ▶` (keys `[ ]` for Start, `, .` for End) and `Reset`, to look at the neighbouring pages; looking saves nothing. When a pane is on a different page than the config, and the sub-sections are written in the file's own entry of a `scan_config.json` inside the configs repo (a git checkout, `base_pdf_path` by default, or `SCAN_CONFIG_EDIT_ROOT`), a **SET start/end = N** button appears. It changes only that one number in the file (formatting untouched, so git shows a one-line change; it never commits) and refuses if the file no longer matches what was on screen. `Undo` puts the old page back. The sub-section list can also be edited: **Remove** deletes the ticked sub-sections (or the one on screen if none is ticked), and their pages are then no longer processed; **Merge** joins two or more neighbouring ticked ones into the first, running from its start page to the last one's end page, under a name you type (refused when their fields or volumes differ, or only some have an `end_side`). Removing every sub-section is refused, since the file would then be indexed as one block. Both edit only the affected items in the file text; the server checks the result against the plan before writing, and the last remove/merge of a file can be undone until the file changes or the dev server restarts. Re-index the file in Discover afterwards; the indexed chunks keep the old pages until then.
