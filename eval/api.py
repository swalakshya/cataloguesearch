...
async def process_scripture_llm(
    ...
    language: str = Form("hin", description="Language code (hin, guj, hin+guj)"),
    ...
    try:
        ...
        # Call LLM extractor. `language` (hin, guj, hin+guj) picks the prompt's
        # Hindi/Gujarati block types, exactly as scan_config's language does at
        # indexing time. `dhundhari` overrides it (Hindi-only) with the prompt
        # variant that also asks the LLM to translate Dhundhari blocks to modern Hindi.
        extraction_language = "dhundhari" if dhundhari else language
        log_handle.info(
...
from backend.crawler.index_status import (  # noqa: E402
    config_hash as _config_hash,
    classify_index_status as _classify_index_status,
    walk_pdfs as _walk_pdfs,
    terminal_scan_config_dir as _get_terminal_scan_config_dir,
)
